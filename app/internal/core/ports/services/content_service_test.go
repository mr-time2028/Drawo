...
func TestContentService(t *testing.T) {
	...
	repo := repositories.NewContentRepo(db); pRepo := repositories.NewProfileRepo(db)
	svc := NewContentService(repo, pRepo, 100); ctx := context.Background()
	...
	assert.True(t, dirty); assert.Equal(t, "hello *** world", cleaned)
}
