...
func TestAuthController(t *testing.T) {
	...
	t.Run("Register_Success", func(t *testing.T) {
		svc.On("Register", mock.Anything, "hamid", "Pass@123").Return(&domain.User{ID: "1", Username: "hamid"}, nil).Once()

		body := `{"username":"hamid", "password":"Pass@123", "confirm_password":"Pass@123"}`
	...
	t.Run("Register_PasswordMismatch", func(t *testing.T) {
		body := `{"username":"hamid", "password":"Pass@123", "confirm_password":"Other@123"}`
		w := httptest.NewRecorder()
		req, _ := http.NewRequest("POST", "/register", bytes.NewBufferString(body))
		router.ServeHTTP(w, req)

		assert.Equal(t, http.StatusBadRequest, w.Code)
		assert.Contains(t, w.Body.String(), "passwords_do_not_match")
	})
