...
// AccountCreator creates the user and its required profile atomically.
// Auth registration uses this optional capability when the concrete repository
// supports it, so a profile creation failure can never leave an orphaned user.
...
func (r *userRepo) CreateUserWithProfile(ctx context.Context, user *domain.User, profile *domain.Profile) error {
...
func createProfile(db *gorm.DB, profile *domain.Profile) error {
	if profile.Email == "" {
		return db.Omit("Email").Create(profile).Error
	}
