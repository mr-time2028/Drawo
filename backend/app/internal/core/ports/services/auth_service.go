...
func (s *authService) Register(ctx context.Context, username, password string) (*domain.User, error) {
	...
	if exists {
		return nil, errors.New(errors.ErrConflict, "username is already taken").WithCode("username_taken").WithField("username")
	}
	...
	// User and profile must be created atomically. A failed profile insert must
	// rollback the user insert, otherwise the client receives a 500 but the
...
func (s *authService) Login(ctx context.Context, username, password, ip, userAgent string) (*domain.TokenPair, error) {
	...
	if !allowed {
		return nil, errors.New(errors.ErrTooManyRequests, "too many failed attempts, please wait").WithCode("too_many_login_attempts")
	}

	// 2. Fetch user
	user, err := s.userRepo.GetByUsername(username)
	if err != nil || user == nil {
		return nil, errors.New(errors.ErrUnauthorized, "invalid username or password").WithCode("invalid_credentials")
	}
...
func (s *authService) Refresh(ctx context.Context, refreshToken string) (*domain.TokenPair, error) {
	...
	if err != nil {
		return nil, errors.New(errors.ErrUnauthorized, "invalid or expired refresh token").WithCode("invalid_refresh_token")
	}
	...
	if err != nil || session == nil {
		...
		return nil, errors.New(errors.ErrUnauthorized, "session no longer active").WithCode("session_inactive")
	}
	...
	if session.RefreshTokenID != claims.TokenID {
		...
		return nil, errors.New(errors.ErrUnauthorized, "security alert: token reuse detected").WithCode("token_reuse_detected")
	}
